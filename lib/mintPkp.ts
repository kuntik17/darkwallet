import { AuthMethodScope, AuthMethodType, LitNetwork } from "@lit-protocol/constants";
...
export const mintPkp = async (telegramUser: TelegramUser) => {
  ...
  try {
    ...
    console.log(`✅ Calculated IPFS CID: ${litActionIpfsCid}. Hexlified version: 0x${Buffer.from(bs58.decode(litActionIpfsCid)).toString("hex")}`);
    ...
    const tx = await litContracts.pkpHelperContract.write.mintNextAndAddAuthMethods(
      ...
      [`0x${Buffer.from(bs58.decode(litActionIpfsCid)).toString("hex")}`, authMethodId], // permittedAuthMethodIds
    ...
    console.log(`✅ Got permitted auth methods: ${JSON.stringify(permittedAuthMethods, null, 2)}`);
...
const getPkpInfoFromMintReceipt = async (txReceipt: ethers.ContractReceipt, litContractsClient: LitContracts) => {
  const pkpMintedEvent = txReceipt!.events!.find((event) => event.topics[0] === "0x3b2cc0657d0387a736293d66389f78e4c8025e413c7a1ee67b7707d4418c46b8");
