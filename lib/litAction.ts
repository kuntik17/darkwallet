...
const _litActionCode = async () => {
  ...
  const TELEGRAM_AUTH_METHOD_TYPE = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("Lit Developer Guide Telegram Auth Example"));
  ...
  try {
    ...
    const secretKeyHash = await crypto.subtle.digest("SHA-256", encoder.encode(telegramBotSecret));
